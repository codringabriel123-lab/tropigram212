...
export default function Layout() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d" }}>
      {/* TOP NAV */}
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: "#0d0d0d", borderBottom: "1px solid #1f1f1f", padding: "10px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        <div onClick={() => navigate("/")} style={{ fontSize: 18, fontWeight: 800, color: "#fff", whiteSpace: "nowrap", cursor: "pointer" }}>
          🌴 <span style={{ color: "#e91e8c" }}>TRP</span>
        </div>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          <input
            ...
            style={{ width: "100%", background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 20, padding: "8px 16px", color: "#fff", fontSize: 13 }}
          />
          {showSearch && searchResults.length > 0 && (
            <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#1a1a1a", border: "1px solid #333", borderRadius: 10, zIndex: 200, overflow: "hidden", boxShadow: "0 8px 24px #00000088" }}>
              {searchResults.map(u => (
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            {showNotif && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, width: 310, background: "#1a1a1a", border: "1px solid #333", borderRadius: 12, zIndex: 300, boxShadow: "0 8px 32px #000000aa", overflow: "hidden" }}>
                <div style={{ padding: "12px 16px", borderBottom: "1px solid #222", fontWeight: 700, fontSize: 14, display: "flex", justifyContent: "space-between" }}>
                  Notificări <span onClick={() => setShowNotif(false)} style={{ cursor: "pointer", color: "#555" }}>✕</span>
                </div>
                {notifs.length === 0 && <div style={{ padding: "2rem", textAlign: "center", color: "#555" }}>Nicio notificare</div>}
                {notifs.slice(0, 10).map(n => (
                  <div key={n._id} style={{ padding: "10px 16px", borderBottom: "1px solid #1f1f1f", background: n.read ? "transparent" : "#1f1020", display: "flex", gap: 10, alignItems: "flex-start" }}>
                    <span style={{ fontSize: 18 }}>{n.type === "like" ? "❤️" : n.type === "follow" ? "👤" : "💬"}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, color: "#ccc" }}>
                        <b>{n.sender?.username}</b> {n.type === "like" ? "a dat like la postarea ta" : n.type === "follow" ? "a început să te urmărească" : "a comentat la postarea ta"}
                      </div>
                      <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{new Date(n.createdAt).toLocaleDateString("ro-RO")}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* BOTTOM NAV */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#0d0d0d", borderTop: "1px solid #1f1f1f", display: "flex", zIndex: 100 }}>
        {navItems.map(tab => {
