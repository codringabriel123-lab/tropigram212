...
export default function ProfilePage() {
  ...
  // Tab activ: "posts" = postări originale, "reposts" = reposturi
  ...
  return (
    <div>
      ...
      {/* Tab-uri: Postări / Reposturi */}
      {(() => {
        const originalPosts = posts.filter(p => !p.repostOf);
        const repostedPosts = posts.filter(p => p.repostOf);
        const visiblePosts = activeTab === "posts" ? originalPosts : repostedPosts;
        return (
          <>
            <div style={{ display: "flex", borderBottom: "1px solid #1f1f1f" }}>
              <button
                ...
                style={{
                  ...
                  borderBottom: activeTab === "posts" ? "2px solid #e91e8c" : "2px solid transparent",
                  color: activeTab === "posts" ? "#e91e8c" : "#666",
                  fontWeight: 700, fontSize: 13, cursor: "pointer",
                }}
              >
                Postări ({originalPosts.length})
              </button>
              <button
                onClick={() => setActiveTab("reposts")}
                style={{
                  flex: 1, padding: "12px 0", background: "transparent", border: "none",
                  borderBottom: activeTab === "reposts" ? "2px solid #e91e8c" : "2px solid transparent",
                  color: activeTab === "reposts" ? "#e91e8c" : "#666",
                  fontWeight: 700, fontSize: 13, cursor: "pointer",
                }}
              >
                🔁 Reposturi ({repostedPosts.length})
              </button>
            </div>
            {visiblePosts.length === 0 ? (
        <div style={{ textAlign: "center", padding: "3rem", color: "#555" }}>
          <div style={{ fontSize: 32, marginBottom: 8 }}>📭</div>
          <div>{activeTab === "posts" ? "Nicio postare originală încă" : "Niciun repost încă"}</div>
        </div>
      ) : (
        visiblePosts.map(p => <PostCard key={p._id} post={p} onDelete={handleDeletePost} />)
      )}
          </>
        );
      })()}
